...
const AclGuard = props => {
  ...
  useEffect(() => {
    if (session.data?.user.role && !guestGuard && router.route === '/') {
      const homeRoute = getHomeRoute(session.data?.user.role)
      router.replace(homeRoute)
    }
  }, [session.data?.user.role, guestGuard, router])
  ...
  // Render Not Authorized component if the current sysUser has limited access
  return (
    <BlankLayout>
      <NotAuthorized />
    </BlankLayout>
