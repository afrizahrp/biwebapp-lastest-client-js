...
const AddEventSidebar = props => {
  ...
  const onSubmit = data => {
    ...
    if (store.selectedPlan === null || (store.selectedPlan !== null && !store.selectedPlan.title.length)) {
      dispatch(addEvent(modifiedEvent))
    } else {
      dispatch(updateEvent({ id: store.selectedPlan.id, ...modifiedEvent }))
    }
  ...
  const handleDeleteEvent = () => {
    if (store.selectedPlan) {
      dispatch(deleteEvent(store.selectedPlan.id))
    }

    // calendarApi.getEventById(store.selectedPlan.id).remove()
    handleSidebarClose()
  }
  ...
  const resetToStoredValues = useCallback(() => {
    if (store.selectedPlan !== null) {
      const event = store.selectedPlan
  ...
  }, [addEventSidebarOpen, resetToStoredValues, resetToEmptyValues, store.selectedPlan])
    ...
    >
      ...
      >
        <Typography variant='h6'>
          {store.selectedPlan !== null && store.selectedPlan.title.length ? 'Update Event' : 'Add Event'}
        </Typography>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          {store.selectedPlan !== null && store.selectedPlan.title.length ? (
            <IconButton
              size='small'
              onClick={handleDeleteEvent}
              sx={{ color: 'text.primary', mr: store.selectedPlan !== null ? 1 : 0 }}
            >
